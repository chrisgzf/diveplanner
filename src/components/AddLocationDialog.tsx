    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Name" aria-label="location name" className="w-full rounded-md border border-line bg-surface-elevated px-2 py-2 text-sm" />
          <input value={country} onChange={(e) => setCountry(e.target.value)} placeholder="Country" aria-label="country" className="w-full rounded-md border border-line bg-surface-elevated px-2 py-2 text-sm" />
          <select value={difficulty} onChange={(e) => setDifficulty(e.target.value as Location['difficulty'])} className="w-full rounded-md border border-line bg-surface-elevated px-2 py-2 text-sm">
          ...
          <div className="grid grid-cols-6 gap-2">
            {ratings.map((r, i) => (
              <select key={i} aria-label={`month ${i + 1} rating`} value={r}
                ...
                className="rounded border border-line bg-surface-elevated px-1 py-1 text-xs">
